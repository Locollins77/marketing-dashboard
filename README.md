...
`server/seed.js`, which populates SQLite with mock campaigns, leads, journey events,
...
- **DB:** SQLite (`better-sqlite3`), file lives in `data/dashboard.sqlite` (gitignored)
...
- **Hosting (planned):** Render, auto-deploying on push to this repo
...
Edit `.env` and set `SESSION_SECRET`, `ADMIN_USERNAME`, and `ADMIN_PASSWORD` to your own values.
